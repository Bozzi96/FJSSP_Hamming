function [gamma_sol, delta_sol, sol, prob] = FJSSP_HammingDistance(G,G_j,P, S0, solutions)
    % Parameters: 
    ...
    % delta [J,J,M] = Disjunctive variables
    ...
    % Define auxiliary binary variables to represent the absolute differences
   ...
   % Final completion time constraints
    ...
    for i = 1:A
        if ~gamma_old(i)
        diffgamma(i) = diff_gamma(i) == gamma(i) - gamma_old(i);
        else
        diffgamma(A+i) = diff_gamma(i) == gamma_old(i) - gamma(i);
        end
    end
    prob.Constraints.diffgamma = diffgamma;
    ...
    for m=1:M
        for i=1:J-1
            for j=i+1:J
                if ~delta_old(i,j,m)
                    diffdelta(i,j,m) = diff_delta(i,j,m) == delta(i,j,m) - delta_old(i,j,m);
                else
                    diffdelta(J+i,J+j,M+m) = diff_delta(i,j,m) == delta_old(i,j,m) - delta(i,j,m);
                end
            end
        end
    end
    prob.Constraints.diffdelta = diffdelta;

    % Calculate the Hamming distance
    hammingDist = sum(diff_gamma) + sum(sum(sum(diff_delta)));
    % Add the constraints for the minimum and maximum Hamming distance
    prob.Constraints.minDist = hammingDist >= 1;
    prob.Constraints.maxDist = hammingDist <= 20;
    ...
    for h=1:size(solutions,2)
        ...
        for i=1:A
            if int8(gamma_old(i)) == 1
                somma_xor = somma_xor + gamma(i);
            else
                somma_xor = somma_xor -BigM*gamma(i);
            end
        end
        for m=1:M
            for i=1:J-1
                for j=i+1:J
                    if int8(delta_old(i,j,m)) == 1
                        somma_xor = somma_xor + delta(i,j,m);
                    else
                        somma_xor = somma_xor -BigM * delta(i,j,m);
                    end
                end
            end
        end
    ...
    prob.Constraints.constraints_xor = constraints_xor;
    
    %% Objective function
    %prob.Objective = hammingDist;
    ...
    [sol,~] = solve(prob,x0,'Options', oldoptions);
    sol.hammingDist = hammingDist;
    if ~isempty(sol)
        gamma_sol = sol.gamma;
        delta_sol = sol.delta;
    else
