function [sol, val] = FJSSP_evaluateGammaDelta(G,G_j,P, S0, gamma, delta)
    % Parameters: 
   ...
   % Final completion time constraints
    ...
    %% Solve problem
    %show(prob)
    oldoptions = optimoptions(@fmincon,'Algorithm','sqp','MaxIterations',5,'Display', 'off');
    [sol,val] = solve(prob,x0, 'Options', oldoptions);
    sol.gamma = gamma;
    sol.delta = delta;

    % Check if sol is feasible
    % feas = 1;
    % % Gamma feasible
    % for j=1:J
    %     idx = find(G_j == j); % for each possible alternative on job j
    %     if sum(gamma(idx)) == 1 % choose only one alternative
    %         continue
    %     else
    %         feas = 0;
    %         return
    ...
    %                    continue
    %                end
    %                if delta(j1,j2,m) + delta(j2,j1,m) == 1 %non sono sicuro che sia == e non <=
    %                    continue
    %                else
    %                    feas = 0;
    %                    return
